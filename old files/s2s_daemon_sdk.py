    ...
    async def handle_transcript_event(self, transcript_event: TranscriptEvent):
        ...
        for result in results:
            if result.is_partial == False:
                ...
                testing = result.alternatives[0].transcript
...
async def mic_stream():
    ...
    mic_loop = asyncio.get_event_loop()
    input_queue = asyncio.Queue()


    def callback(indata, frame_count, time_info, status):
        mic_loop.call_soon_threadsafe(input_queue.put_nowait, (bytes(indata), status))

        # Be sure to use the correct parameters for the audio stream that matches
        # the audio formats described for the source language you'll be using:
        # https://docs.aws.amazon.com/transcribe/latest/dg/streaming.html
    channelcount = device_info["max_input_channels"] \
    if (device_info["max_output_channels"] < device_info["max_input_channels"]) \
    else device_info["max_output_channels"]
